...
We initialize from \texttt{microsoft/deberta-v3-base} (184M parameters) and add a binary classification head. Training uses AdamW (lr=$2 \times 10^{-5}$), batch size 8, 3 epochs, max length 512, float32 precision, on NVIDIA P100 (Kaggle).
...
The TF-IDF baseline was evaluated on the HaluEval QA subset (10,000 pairs expanded to 20,000 balanced samples) by Debasmita Biswas.
...
\begin{enumerate}
    ...
    \item \textbf{TF-IDF baseline (Debasmita):} 21\% accuracy on HaluEval QA confirms lexical features are anti-predictive.
    ...
    \item \textbf{Model deployment:} Fine-tuned model on HuggingFace (\texttt{varunchundru/hallucination-detector-deberta}).
\end{enumerate}
